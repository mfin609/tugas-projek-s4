...
export default function StoreClient({ initialProducts = [], initialOutlets = [] }) {
        ...
        <div className="container position-relative" style={{ zIndex: 1 }}>
          <div className="row align-items-center">
            ...
            <div className="col-lg-6">
              <div className="hero-img-wrap fade-up fade-up-delay-2" style={{ position: "relative" }}>
                  <img
                    src="/api/images/hero_banner.png"
                    alt="Premium Artisan Bakery Roti Yuki"
                    className="w-100 object-fit-cover shadow-lg border border-4 border-white"
                    style={{
                      height: "420px",
                      borderRadius: "24px"
                    }}
...
function ProductCard({ product, onAddToCart }) {
  const initialIdx = useMemo(() => {
    const idx = product.stocks.findIndex(s => s.stock > 0);
    return idx === -1 ? 0 : idx;
  }, [product.stocks]);

  const [selectedStockIdx, setSelectedStockIdx] = useState(initialIdx);

  const selectedStock = product.stocks[selectedStockIdx];
  const hasAvailable = product.stocks.some((s) => s.stock > 0);
  ...
  return (
    <div className="product-card" id={`product-card-${product.id}`}>
      {imageUrl ? (
        <img src={imageUrl} alt={product.title} className="product-card-img-placeholder w-100" style={{ objectFit: "cover", display: "block" }} />
      ) : (
              ...
              {product.stocks.length > 1 && (
                ...
                >
                  {product.stocks.map((s, i) => (
                    <option key={s.id} value={i} disabled={s.stock === 0}>
                      {s.outlet?.name?.replace("Toko Roti Yuki - ", "") || "Outlet"} {s.stock === 0 ? "(Habis)" : ""}
                    </option>
                  ))}
                </select>
              )}
